-- schema_v2_fixed.sql
-- SQLite 3.37.2+ compatible - PRODUCTION READY
--
-- FTS5 CONTENTLESS design (porządne rozwiązanie, nie półśrodek)
-- Ręczne zarządzanie FTS przez triggery = pełna kontrola + możliwość JOIN z docs
--
-- Usage: sqlite3 sqlite_knowledge.db < schema_v2_fixed.sql
--
...
--
-- Changes from v2.0:
--   - FTS5 changed to contentless (content='')
--   - Triggers now explicitly INSERT/UPDATE/DELETE FTS data
--   - topic_id and module properly pulled from docs table
...
CREATE TABLE sessions (
    ...
    metadata        TEXT  -- JSON (TEXT for compatibility with 3.37.2, BLOB/JSONB for 3.51.0+)
);
...
-- ==============================================================================
-- FTS5 - CONTENTLESS MODE (proper solution)
-- ==============================================================================
--
-- Why contentless?
-- - chunks table doesn't have topic_id/module (they're in docs)
-- - We need to JOIN docs to get those values
-- - contentless gives us full control via triggers
-- - No data duplication issues
-- - Can add/remove FTS columns without changing source table
--
...
-- (contentless FTS5 requires special 'delete' command)
...
-- (contentless FTS5 requires special 'delete' command, not regular DELETE)
...
-- (important for data consistency when doc metadata changes)
-- (contentless FTS5 requires special 'delete' command for each chunk)
...
SELECT '✓ Schema v2.1 loaded (FIXED FTS5)!' AS status,
       '✓ FTS5: contentless mode with proper triggers' AS fts,
       '✓ Tables: topics, sessions, messages, docs, chunks' AS tables,
       '✓ Triggers: chunks_fts_ai, chunks_fts_au, chunks_fts_ad, docs_fts_au' AS triggers,
...
-- Search FTS:
-- SELECT c.text, d.title, d.module
-- FROM chunks_fts fts
-- JOIN chunks c ON c.id = fts.rowid
-- JOIN docs d ON d.id = c.doc_id
-- WHERE chunks_fts MATCH 'checkpoint'
-- ORDER BY fts.rank LIMIT 10;

-- Search with module filter (using UNINDEXED column):
-- SELECT c.text
-- FROM chunks_fts fts
-- JOIN chunks c ON c.id = fts.rowid
-- WHERE chunks_fts MATCH 'wal'
--   AND fts.module = 'PRAGMA'
-- LIMIT 10;
